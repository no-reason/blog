...
&考虑集合S=\{gHg^{-1}|g\in G\}\\
&考虑G在H上的共轭作用\\
&|G|=|Stab_{G}(H)|\times |Orb_{G}(H)|\\
&此时Orb_{G}(H)等于S\\
&Stab_{G}(H)等于N_{G}(H),即H在G中的正规化子\\
&\Rightarrow |G|=|S|\times |N_{G}(H)|(H在G中的正规化子)\\
&|N_{G}(H)|中一定不含有p\\
&故|S|一定是p的倍数\\
&\\\\
&考虑N在S上的作用:\\
...
1.任取一个Sylowp子群N,考虑其轨道Orb(N),考虑任意一个p-子群H,考虑H通过共轭作用作用到N上去,利用轨道分解方程将Orb(N)做分解,可以发现Orb(N)有一个在H作用下的不动点,证明x\in 这个不动点
2.先取一个Sylow p 子群,考虑它在共轭作用下的Orb,然后用另一个Sylow p子群对这个Orb做共轭作用，然后还是轨道分解，证明有不动点就行

3.证明P作用到所有sylow-p 子群构造的轨道上的不动点是唯一的

$
\begin{aligned}
&推论:交换群的Sylow ~~p子群是唯一的\\
&\\
&推论:\\
&有限Abel群可以表示为它的Sylow ~~p子群的直和\\
&\\
&结论:\\
&设p,q是互异的素数\\
&证明pq阶交换群是循环群\\
...
&N_{7}的可能情况为1或8\\
&N_{2}的可能情况为1或2\\
&若7阶子群只有一个,则该7阶子群为正规子群\\
&G一定不是单群\\
&同理8阶子群也不可能只有一个\\
&8个7阶子群一共有1+8\times 6=49个元素\\
&剩下7个2^{k}阶元素\\
&因此这7个元素只能聚在一起成为一个Sylow-2 子群\\
&矛盾\\
&故知原命题成立\\
&\\
&证明6阶群只有Z_6和S_3两种结构\\
...
&设G是交换-p群,\\
&a是G中阶最大的元素,H=<a>\ne G\\
&若\overline{b}是G/H的p^{r}阶元素\\
&证明在G中存在一个p^{r}阶元素c,s.t. \overline{c}=\overline{b}\\
&设|a|=p^{t},|b|=p^{s}\\
&则r\leq s\leq t\\
&|\overline{b}|=p^{r}\\
&\Rightarrow (\overline{b})^{p^r}=e\in G/H\\
&\Rightarrow b^{p^r}=a^{k}\\
&\\
&断言:\\
&这时p^{r}\mid k\\
&e=b^{p^s}=(b(^{p^r})^{p^{s-r}})\\
&=a^{kp^{s-r}}\\
&\Rightarrow p^{t-s+r}\mid k\\
&\Rightarrow p^{r}\mid k\\
&\\
&我们需要找到合适的x,s.t. \\
&ba^{x}的阶为r\\
&考虑必要性假设\\
&e=b^{p^r}a^{xp^r}=a^{k+xp^r}\\
&\Rightarrow p^{t}\mid k+xp^{r}\\
&以简驭繁\\
&令x=p^{t-r}-kp^{-r}\\
&若该数的阶小于p^{r}\\
&直接带入计算可知矛盾\\
&\\\\\\\\\
&交换p-群G与有限个循环p-群的直和同构\\
&若G同构于G_{k_1}\times G_{k_2}\times \cdots \times G_{k_r}\\
&其中G_{k_i}是p^{k_i}阶循环群群\\
&则整数序列k_1\geq k_2\geq\cdots \geq k_r\geq 1被G唯一确定\\
&\\\\
&对群的阶进行归纳\\
&设|G|=p^{k}\\
&k=1时原命题显然成立\\
...
&设H=<a_1>\\
&考虑G/H,故由归纳假设知\\
&G/H\cong \overline{G_{k_2}}\times \cdots \times \overline{G_{k_r}}\\
&设\overline{c_i}是\overline{G_{k_i}}的生成元\\
&由引理9.1\\
&\exists G的p^{k_i}阶元a_i\\
&s.t. \overline{c_i}=\overline{a_i}\\
&令G_{k_i}=<a_i>\\
&下面证明:G=G_{k_1}\times G_{k_2}\times \cdots \times G_{k_r}\\
&\\\\
&
\end{aligned}
$
...
&e.g.1,6阶群的同构类:\\
&N_{3}=1,N_{2}=1,3\\
&若N_{2}=1,则G\cong Z_{2}\times Z_{3}\cong Z_{6}\\
&若N_{2}=3,N_{3}=1\\
&设<a>,<b>,<c>\in Syl_{2}(G)\\
&<x>\in Syl_{3}(G)\\
&由<x>\lhd G\\
